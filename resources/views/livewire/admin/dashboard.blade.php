<div>
    ...
    <div x-data="{
        setHoy() {
            ...
            $wire.set('fechaDesde', hoy);
            $wire.set('fechaHasta', hoy);
        },
        setEstaSemana() {
            let hoy = new Date();
            let lunes = new Date(hoy);
            lunes.setDate(hoy.getDate() - hoy.getDay() + 1);
            $wire.set('fechaDesde', lunes.toISOString().split('T')[0]);
            $wire.set('fechaHasta', hoy.toISOString().split('T')[0]);
        },
        setEsteMes() {
            let hoy = new Date();
            let primero = new Date(hoy.getFullYear(), hoy.getMonth(), 1);
            $wire.set('fechaDesde', primero.toISOString().split('T')[0]);
            $wire.set('fechaHasta', hoy.toISOString().split('T')[0]);
        }
    }" class="flex flex-wrap items-end gap-3 mb-6 p-4 bg-white rounded-2xl shadow-sm border border-[#d4b896]/20">
        <div>
            ...
            <input wire:model.live="fechaDesde" type="date"
                   class="border border-[#d4b896]/40 bg-[#faf6f0] rounded-xl px-3 py-2 text-sm text-[#2c1a0e] shadow-sm
        ...
        <div class="flex items-center gap-2 pb-0.5">
            <button @click="setHoy()"
                    class="text-[11px] font-medium px-3 py-2 rounded-lg border border-[#d4b896]/40 text-[#8b5e3c] hover:bg-[#f0e9de] hover:text-[#2c1a0e] transition-all duration-200">
                Hoy
            </button>
            <button @click="setEstaSemana()"
                    class="text-[11px] font-medium px-3 py-2 rounded-lg border border-[#d4b896]/40 text-[#8b5e3c] hover:bg-[#f0e9de] hover:text-[#2c1a0e] transition-all duration-200">
                Esta semana
            </button>
            <button @click="setEsteMes()"
                    class="text-[11px] font-medium px-3 py-2 rounded-lg border border-[#d4b896]/40 text-[#8b5e3c] hover:bg-[#f0e9de] hover:text-[#2c1a0e] transition-all duration-200">
                Este mes
            </button>
            @if($fechaDesde || $fechaHasta)
                <button @click="$wire.set('fechaDesde', ''); $wire.set('fechaHasta', '');"
                        class="text-[11px] font-medium px-3 py-2 rounded-lg text-red-400 hover:text-red-600 hover:bg-red-50 transition-all duration-200 border border-red-200/50">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-[#d4b896]/20 overflow-hidden">
            <div class="px-6 py-4 border-b border-[#d4b896]/20 flex items-center justify-between">
                ...
                <a href="{{ route('admin.pedidos') }}" wire:navigate class="text-[12px] text-[#386641] hover:text-[#2d5534] font-medium transition-colors flex items-center gap-1">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-[#d4b896]/20 overflow-hidden">
            <div class="px-6 py-4 border-b border-[#d4b896]/20 flex items-center justify-between">
                ...
                <a href="{{ route('admin.productos') }}" wire:navigate class="text-[12px] text-[#386641] hover:text-[#2d5534] font-medium transition-colors flex items-center gap-1">
    ...
    <div class="mt-6 bg-white rounded-2xl shadow-sm border border-[#d4b896]/20 overflow-hidden">
        <div class="px-6 py-4 border-b border-[#d4b896]/20 flex items-center justify-between">
            <h2 class="text-base text-[#2c1a0e] flex items-center gap-2" style="font-family:'DM Serif Display',serif;">
                ...
                @if($fechaDesde && $fechaHasta)
                    Actividad — {{ \Carbon\Carbon::parse($fechaDesde)->format('d/m') }} al {{ \Carbon\Carbon::parse($fechaHasta)->format('d/m') }}
                @else
